...
func NewPSQLClient(opts PSQLOptions) (*Client, error) {
	...
	// ФИКС Подключение к db
	// Создаем Ent драйвер из sql.DB
	drv := entsql.OpenDB(dialect.Postgres, db)
	clientOpts = append(clientOpts, Driver(drv))
	// ФИКС: Включение debug режима при соответствующей опции
	if opts.debug {
...
func NewPgxDB(opts PgxOptions) (*sql.DB, error) {
	...
	dsn := fmt.Sprintf("host=%s port=%s user=%s password=%s dbname=%s sslmode=disable",
		host, port, opts.username, opts.password, opts.database)
