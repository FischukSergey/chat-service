...
func (cc *ChatCreate) check() error {
	...
	if v, ok := cc.mutation.ClientID(); ok {
		if err := chat.ClientIDValidator(v.String()); err != nil {
	...
	if _, ok := cc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`store: missing required field "Chat.updated_at"`)}
	}
...
func (cc *ChatCreate) createSpec() (*Chat, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.ClientID(); ok {
		_spec.SetField(chat.FieldClientID, field.TypeString, value)
