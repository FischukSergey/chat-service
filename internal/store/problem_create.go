...
func (pc *ProblemCreate) SetStatus(pr problem.Status) *ProblemCreate {
	pc.mutation.SetStatus(pr)
	return pc
}

// SetNillableStatus sets the "status" field if the given value is not nil.
func (pc *ProblemCreate) SetNillableStatus(pr *problem.Status) *ProblemCreate {
	if pr != nil {
		pc.SetStatus(*pr)
	}
	return pc
...
func (pc *ProblemCreate) check() error {
	...
	if v, ok := pc.mutation.ManagerID(); ok {
		if err := problem.ManagerIDValidator(v.String()); err != nil {
			return &ValidationError{Name: "manager_id", err: fmt.Errorf(`store: validator failed for field "Problem.manager_id": %w`, err)}
		}
	}
	if _, ok := pc.mutation.Status(); !ok {
		return &ValidationError{Name: "status", err: errors.New(`store: missing required field "Problem.status"`)}
	}
	...
	if _, ok := pc.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`store: missing required field "Problem.updated_at"`)}
	}
...
func (pc *ProblemCreate) createSpec() (*Problem, *sqlgraph.CreateSpec) {
	var (
		...
		_spec = sqlgraph.NewCreateSpec(problem.Table, sqlgraph.NewFieldSpec(problem.FieldID, field.TypeString))
	)
	...
	if value, ok := pc.mutation.ManagerID(); ok {
		_spec.SetField(problem.FieldManagerID, field.TypeString, value)
